...
import { Card, CardContent, CardHeader, Accordion, AccordionDetails, AccordionSummary, makeStyles } from "@material-ui/core";
    ...
    return (
        <Card className={cardClasses}>
            ...
            <CardContent className={cardContentClasses}>
                {canExp ?
                    (
                        <Accordion onChange={handleToggle} disabled={!canExp} expanded={exp} className={classes.accordionRoot}>
                            <AccordionSummary classes={{ content: classes.content, root: classes.summaryRoot }} expandIcon={<ExpandMoreIcon />}>
                                {(exp || (!exp && !header)) &&
                    ...
                    (
                        <Accordion onChange={handleToggle} expanded={false} className={classes.accordionRoot}>
                            <AccordionSummary classes={{ content: classes.content, root: classes.summaryRoot }}  >
                                {(exp || (!exp && !header)) &&
                                    <Typography variant="subtitle1" className={classes.cardTitle}>
                                        {title}
                                    </Typography>
